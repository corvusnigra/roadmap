/**
 * Одноразовый scaffolder MDX-узлов для роли «Java Middle Interview».
 * Импортирует структурированный курс из ./java-middle-curriculum и
 * для каждой из 36 тем эмитит MDX с реальным телом теории
 ...
 * полей темы (diff, markers, anti, practice), а не выдуманы.
 *
 * Структурно идентичен scaffold-levenchuk.ts; различаются только источник
 * данных и LEVELS_META. Когда появится JAVA_MIDDLE_MASTERY (manual MCQ),
 * добавится override-логика (см. levenchuk-mastery).
 *
 *   pnpm tsx src/scripts/scaffold-java-middle.ts
 */
...
import {
  JAVA_MIDDLE_TOPICS,
  JAVA_LEVELS_META as LEVELS_META,
  type LDiscipline,
} from "@/scripts/java-middle-curriculum";

const ROLE_SLUG = "java-middle-interview";
...
function buildMasteryQuizStubs(d: LDiscipline): McqStub[] {
  // Java Middle: ручных MCQ пока нет, генерируем из полей темы.
  // Когда появится JAVA_MIDDLE_MASTERY — добавить override-логику здесь.
  const out: McqStub[] = [];
...
async function main() {
  ...
  let created = 0;
  let skipped = 0;
  for (const d of JAVA_MIDDLE_TOPICS) {
...
if (process.argv[1] && process.argv[1].includes("scaffold-java-middle")) {
  main().catch((err) => {
    console.error(
      "scaffold-java-middle failed:",
